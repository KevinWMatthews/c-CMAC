...
TEST_GROUP(Aes128_HandleComparator)
{
    ...
    AES128_HANDLE params;
    AES128_HANDLE params2;
...
TEST(Aes128_HandleComparator, object_to_string_shows_size_and_contents_of_key)
{
    uint8_t key[16] = {};
    params.key = key;
...
TEST(Aes128_HandleComparator, object_to_string_will_not_segfault_with_null_params)
{
...
TEST(Aes128_HandleComparator, object_to_string_will_not_segfault_with_null_pointer)
{
    params.key = NULL;
    params.key_len = 1;
...
TEST(Aes128_HandleComparator, object_to_string_shows_size_and_contents_of_iv)
{
    uint8_t iv[16] = {};
    params.iv = iv;
...
TEST(Aes128_HandleComparator, compare_true_with_empty_params)
{
    CHECK_TRUE( comparator.isEqual(&params, &params2) );
}

TEST(Aes128_HandleComparator, compare_true_with_zero_length_key_and_iv)
{
    params.key_len = 0;
    params.iv_len = 0;
    params2.key_len = 0;
...
TEST(Aes128_HandleComparator, example_use_case)
{
    ...
    mock().expectOneCall("example_use_case")
        .withParameterOfType("AES128_HANDLE", "params", &params);
    ...
    // Fails
    mock().actualCall("example_use_case")
        .withParameterOfType("AES128_HANDLE", "params", &params2);
#endif
}
